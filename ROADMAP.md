...
- **Consent + anonymisation primitives** in Cadence: ✅ **consent gate** — required, voluntary/anonymous/withdrawable notice before every survey (default text, overridable per study via `consentText`), `consentedAt` timestamped on each response (done 2026-05-30); participant-code-only identification (already the model); ✅ **export-and-purge** — a "Delete my responses" path on the completion screen erases the participant's wave record + their rows in the study aggregate (confirm-gated, nudges export first). Done 2026-05-30.
